...
class KDTree
{
  ...
  private:
     ...
     */
    unsigned partition(vector<Point<Dim>> & list, unsigned left, unsigned right, unsigned pivotIndex, unsigned dim);
    Point<Dim> & quickSelect(vector<Point<Dim>> & list, unsigned left, unsigned right, unsigned k, unsigned dim);
    typename KDTree<Dim>::KDTreeNode* buildTree(vector<Point<Dim>> & points, unsigned dim, unsigned left, unsigned right) ;
    void copy(KDTreeNode *& subroot, KDTreeNode * node);
    void deleteHelper(KDTreeNode * node);
    void findNearestNeighbor(KDTreeNode * node, const Point<Dim> & query, Point<Dim> & near, double & distance, int dim) const;
};
