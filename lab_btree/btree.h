...
size_t insertion_idx_helper(const std::vector<T>& elements, const C& val, int start, int end)
{
    if (start == (end - 1) && val > elements[start] && val < elements[end]) {
        return end; 
    }
    T mid = elements[(start + end) / 2];
    if (val == mid) {
        return (start+end) /2;
    } else if (val > mid) {
        return insertion_idx_helper(elements, val, (start+end) / 2, end);
    } else {
        return insertion_idx_helper(elements,val,start, (start+end)/2);
    }
