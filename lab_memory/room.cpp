...
Room::Room(const std::string& init_name, int init_capacity)
    : name(init_name),
      ...
      letterCount(0)
{
    letters = new Letter[max_letters];              //mallocd at
}
...
{
    letters[letterCount++] = L;                              // invalid write of size 8
    count += L.count;
}
...
void Room::print(std::ostream & stream /* = std::cout */)
{
    ...
    for (int L = 0; L < letterCount; L++)
        stream << " " << letters[L].letter;         // invalid read of size 1
    stream << std::endl;
}
